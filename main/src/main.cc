...
//TECHNICALLLYYYYYYYYYYYY Schedulables can store a weak pointer to the group that scheduled them and just set that at the beginning, instead of capturing it by reference
/* To do list:
	Remove FunctionManager
	Probably remove InitializeGroup
	Add its dictionaries to GroupBase(schedulables and endBehavior)
	...
	Add dictionary of list of functions that run when a schedulable is over to FunctionManager
	Add initialized flag to GroupBase IDs
	Stop removing from FunctionManager when schedulables are done
	Add reset functions to initialize of schedulables and reset initialize flag after things are run and figure out general initialize stuff
	Change Schedule to keep track of current index for each system instead of just removing
	Change deconstructor of GroupBase to clean up Schedulables
	Maybe make ProgramScheduler remove schedulables from the schedule when they are finished (virtual bool cleanup that can be overriden by Scheduler)
...
#pragma region SequentialGroupTest1
...
#pragma endregion
	...
	UltraDisplayInitialization->SetInitializationSchedulable(Display.CreateCommand("Initialize of Initialize"));
	UltraDisplayInitialization->SetCleanupSchedulable(Display.CreateCommand("Cleanup of Initialize"));
	UltraDisplay.SetInitialization(UltraDisplayInitialization);
	auto UltraDisplayCleanup = Display.CreateCommand("Cleanup");
	UltraDisplayCleanup->SetCleanupSchedulable(Display.CreateCommand("Cleanup of Cleanup"));
	UltraDisplayCleanup->SetInitializationSchedulable(Display.CreateCommand("Initialize of Cleanup"));
	...
	//SequentialGroup* sequentialGroup = new SequentialGroup({
	//new Schedulable([&]() {std::cout << "Moving 1000 milliseconds\n"; return robot.GetLeftMotor()->UpdateTime(); }, (unsigned char)Systems::LeftMotor),
	//new Schedulable([&]() {return robot.GetLeftMotor()->MoveByTime(1000); }, (unsigned char)Systems::LeftMotor),
	//new Schedulable([&]() {std::cout << "Moving 1000 milliseconds\n"; return robot.GetLeftMotor()->UpdateTime(); }, (unsigned char)Systems::LeftMotor),
	//new Schedulable([&]() {return robot.GetLeftMotor()->MoveByTime(1000); }, (unsigned char)Systems::LeftMotor),
	//new Schedulable([&]() {std::cout << "Moving 1000 milliseconds\n"; return robot.GetLeftMotor()->UpdateTime(); }, (unsigned char)Systems::LeftMotor),
	//new FunctionManage
	...
	//	new Schedulable([&]() {std::cout << "Moving 5000 milliseconds\n"; return robot.GetLeftMotor()->UpdateTime(); }, (unsigned char)Systems::LeftMotor),
	//	MoveLeftMotorByTime.ScheduleWith(fiveThousand),
	//	new Schedulable([&]() {std::cout << "Moving 1000 milliseconds\n"; return robot.GetLeftMotor()->UpdateTime(); }, (unsigned char)Systems::LeftMotor),
	//	MoveLeftMotorByTime.ScheduleWith(oneThousand),
	//	});r::Schedulable([&]() {return robot.GetLeftMotor()->MoveByTime(1000); }, (unsigned char)Systems::LeftMotor),
	//	});

	std::vector<std::shared_ptr<Schedulable>> delayFunctions = std::vector<std::shared_ptr<Schedulable>>();
	...
	//cat.Age = 100;
	// 
	//std::shared_ptr<Schedulable> command = TakesCatPointer.CreateCommand(new Cat(12, "John"));
	...
	std::shared_ptr<Schedulable> newLineFunction = std::make_shared<Schedulable>([&]() {std::cout << std::endl; return true; }, Systems::Five);
...
#pragma region ConditionSchedulableTests
	int basicConditionSchedulableTestInt = 0;
	std::shared_ptr<SequentialGroup> basicConditionSchedulableTest = std::make_shared<SequentialGroup>(SequentialGroup({
		Increment.CreateCommand(basicConditionSchedulableTestInt),
		std::make_shared<ConditionWrapper>(Display.CreateCommand("YYAYAY, SHOULD RUN"), [&]() {return basicConditionSchedulableTestInt > 0; }),
		std::make_shared<ConditionWrapper>(Display.CreateCommand("NOOOOO, SHOULDNT RUN"), [&]() {return basicConditionSchedulableTestInt < 0; }),
		}));
	...
	//loopOverConditionTest->SetInitializationSchedulable(SetInt.CreateCommand(loopOverConditionTestInt, 0));

	std::shared_ptr<Schedulable> LoopCoreSchedulable = LoopCore.CreateCommand();
	LoopCoreSchedulable->SetInitializationSchedulable(SetInt.CreateCommand(loopOverConditionTestInt, 0));
	std::shared_ptr<LoopGroup> loopOverConditionTest2 = std::make_shared<LoopGroup>(LoopGroup({
		std::make_shared<ConditionWrapper>(ConditionWrapper(LoopCoreSchedulable, [&]() {return loopOverConditionTestInt < 5; }))
		}, 10));
	//loopOverConditionTest2->SetInitializationSchedulable(SetInt.CreateCommand(loopOverConditionTestInt, 0));
#pragma endregion
...
#pragma endregion
	...
	std::shared_ptr<Schedulable> endFunction = std::make_shared<Schedulable>([&]() {std::cout << otherMessage << Timer::GetInstance().ElapsedMilliseconds() << std::endl; return true; }, (unsigned char)Systems::All);
	...
	*/
	...
	//Make sure schedulables are destructed properly in the main Scheduler
	//scheduler.Schedule(parallelGroupCombinationTest);
	//scheduler.Schedule(tempSequentialGroupCombinationTest);
...
//new ParallelGroup(std::vector<Schedulable*>({
//	new ScheduledCommand([&]() {return MotorA.MultiStepMove(3); }, (unsigned char)Systems::MotorA),
//	new ScheduledCommand([&]() {return MotorA.ResetCurrentStep(); }, (unsigned char)Systems::MotorA),
//	})),
